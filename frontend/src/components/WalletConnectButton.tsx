...
import { CHAIN_IDS, customSomniaTestnet, customElectroneumTestnet, customBaseMainnet, customBaseSepolia } from "@/config/chains";
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        >
          ...
          <span className="text-xs opacity-60">
            ({chainId === CHAIN_IDS.BASE_SEPOLIA ? 'Base Sepolia' : chainId === CHAIN_IDS.BASE ? 'Base Mainnet' : chainId === CHAIN_IDS.SOMNIA ? 'Somnia Testnet' : chainId === CHAIN_IDS.ELECTRONEUM ? 'Electroneum Testnet' : 'Unknown Network'})
          </span>
          {justCopied ? (
            <svg
        ...
        {dropdownOpen && (
          <div className="absolute right-0 mt-2 w-56 bg-white/10 backdrop-blur-md border border-white/20 rounded-xl shadow-xl z-50">
            <div className="py-2">
              ...
              <button
                onClick={() => handleSwitchNetwork(CHAIN_IDS.BASE_SEPOLIA)}
                className={`flex items-center justify-between w-full px-4 py-2 text-sm hover:bg-white/10 text-left ${
                  chainId === customBaseSepolia.id ? 'text-blue-400' : 'text-white/90'
                }`}
              >
                <span>Base Sepolia</span>
                {chainId === customBaseSepolia.id && (
